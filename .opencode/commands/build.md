---
description: "Execute an epic or stories end-to-end: create story → dev → review → PR. Iterates through all stories with branch stacking. Use when user says 'build epic [N]', 'execute stories', 'start the pipeline', or 'build'. Args: <epic number or story list> [auto|interactive]"
agent: hugo
---
...
You run a 3-agent pipeline per story: **Florent** writes the story, **Romain** implements it, **Victor** reviews and creates the PR. You launch them sequentially using the **Task tool** (subagents `florent`, `romain`, `victor`), stacking branches so PRs can be merged in order.
...
**Critical rule:** Subagents have NO access to your context. You MUST resolve ALL variables (file paths, branch names, story IDs) to their literal values BEFORE passing them in the Task prompt. Never pass unresolved placeholders.
...
- **Deep review flag** (optional): `--deep-review`
  - Forces the Stage 4 deep review pass regardless of the repos.yaml policy.
...
A template ships at `repos.yaml.example` in the codelikehugo repo.
...
If AGENTS.md (or CLAUDE.md) exists at project root, note its path — agents must read it too.
...
Use the **Task tool** to launch the **`florent`** subagent with the following prompt (resolve all values first):
...
Read these files and extract what is relevant to story {resolved_story_id}:
  ...
  - {resolved_agents_md_path} — project rules (AGENTS.md or CLAUDE.md, if it exists)
...
Use the **Task tool** to launch the **`romain`** subagent with the following prompt (resolve all values first):
...
Project rules:   {resolved_agents_md_path} (AGENTS.md or CLAUDE.md — read this for project rules)
...
2. Read project-context.md and the project rules file for conventions and rules
...
- Follow conventions from project-context.md and the project rules file strictly
...
Use the **Task tool** to launch the **`victor`** subagent with the following prompt (resolve all values first):
...
2. Read the project rules file for conventions (commit conventions, etc.)
...
— running an independent review pass on PR `{pr_url}`"

Launch a fresh **`victor`** subagent via the Task tool as an
independent deep-review pass. This is a second opinion, deliberately
separate from the Stage 3 run. Give it:

```
Independent deep review of a PR. You have NOT seen this code before —
review it with fresh eyes. Do not fix anything; report findings only.
...
Run: git -C {resolved_code_root} diff {resolved_base_branch}..{resolved_current_branch}

Review adversarially:
- Correctness: logic errors, race conditions, missing error handling
- Security: injection, auth gaps, secrets
- Spec compliance: every AC implemented AND tested
- Testing: weak assertions, missing edge cases

IMPORTANT: Your final message MUST be findings grouped by severity
(HIGH / MEDIUM / LOW), or "No findings." if clean.
```

When it returns:

- If the deep review surfaces **HIGH-severity findings**, launch
...
- If the deep review fails, log a warning like
...
### Variable Resolution
Resolve ALL variables to literal values BEFORE launching any subagent. Subagents cannot see your context — they only know what you put in their prompt. Double-check that every path, branch name, and story ID is a real value, not a placeholder.
